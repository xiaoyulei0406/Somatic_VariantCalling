...
workflow WES {
	...
	scatter (i in range(length(sample_info))) {
		...
		call mu.variant_call_tn as mu_tn {
			input: tumor_dedup_bam = dup_tumor.bam, normal_dedup_bam = dup_normal.bam, tumor_recal_table = bqsr_tumor.recal_table , normal_recal_table = bqsr_normal.recal_table , tumor_name = vs_list[i][0], normal_name = vs_list[i][1],  Ref_file = Ref, dbsnp = dbsnp
		}
